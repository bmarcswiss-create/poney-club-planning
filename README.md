...
import { Calendar as CalendarIcon, MapPin, Award, Trash2, X, Plus, AlertCircle, Users, Tent, ChevronLeft, ChevronRight, Settings, Edit, Flag } from 'lucide-react';

// --- DONNÉES PAR DÉFAUT ---
const defaultEquipe = [
  { id: 1, nom: "Jean", role: "Palefrenier", total: 25, repos: "" },
  { id: 2, nom: "Marie", role: "Palefrenier", total: 25, repos: "" },
  { id: 3, nom: "Emma", role: "Apprentie", total: 25, repos: "" },
...
export default function App() {
  ...
  const [anneeActuelle, setAnneeActuelle] = useState(() => Number(localStorage.getItem('poney_annee')) || 2026);
  const [dateDuJour, setDateDuJour] = useState("");
  const [selectedDate, setSelectedDate] = useState(todayStr);
  
  const [membresBase, setMembresBase] = useState(() => {
    const saved = localStorage.getItem('poney_equipe');
    return saved ? JSON.parse(saved) : defaultEquipe;
  });

  const [conges, setConges] = useState(() => {
    const saved = localStorage.getItem('poney_conges');
    return saved ? JSON.parse(saved) : [];
  });

  const [evenements, setEvenements] = useState(() => {
    const saved = localStorage.getItem('poney_evenements_v3');
    return saved ? JSON.parse(saved) : evenementsInitiaux;
  });
  ...
  useEffect(() => { localStorage.setItem('poney_equipe', JSON.stringify(membresBase)); }, [membresBase]);
  useEffect(() => { localStorage.setItem('poney_conges', JSON.stringify(conges)); }, [conges]);
  useEffect(() => { localStorage.setItem('poney_annee', anneeActuelle.toString()); }, [anneeActuelle]);
  useEffect(() => { localStorage.setItem('poney_evenements_v3', JSON.stringify(evenements)); }, [evenements]);

  useEffect(() => {
    // CORRECTION ICI : "long" au lieu du format string pour éviter les erreurs TypeScript Strict de Vercel
          ...
          <div className="p-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              {jours.map((jour, jIndex) => {
                ...
                return (
                  <div key={jIndex} onClick={handleDayClick} title={evtFixe ? evtFixe.titre : ''}
  ...
  }, [anneeActuelle, evenements, congesParDate, palefrenierIds, todayStr]); 
