  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] text-[#1B2A49] pb-32 font-sans">
      <header className="bg-[#1B2A49] text-white px-6 pt-10 pb-8 rounded-b-[40px] shadow-xl text-center relative">
        ...
        {/* BOUTON "+" CARRÉ VERT IDENTIQUE ACCUEIL */}
        <button 
          onClick={() => setView('semaine')}
          className="absolute top-8 right-6 bg-[#8DC63F] text-[#1B2A49] p-2.5 rounded-xl shadow-lg hover:scale-105 active:scale-90 transition-all border border-[#8DC63F]/50"
        >
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-6">
        {view === 'jour' ? (
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-6 bg-gray-50/50 border-b flex justify-between items-center text-left">
                      ...
                      <tr key={horse.id} className={`border-b border-gray-50 transition-all ${isValidated ? 'opacity-30 bg-gray-50/50' : ''}`}>
                        ...
                        {ACTIONS.map(action => {
                          ...
                          if (activeTab === 'proprios') {
                            ...
                            isPlanned = horse.planning?.[jourActuelNom] === action || horse.planning?.[jourActuelNom] === 'Sortie';
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
             <div className="bg-[#1B2A49] p-10 rounded-[50px] text-white shadow-2xl text-left border-b-[10px] border-[#8DC63F]">
                <h3 className="font-black uppercase text-xs mb-8 tracking-[0.2em] text-[#8DC63F]">Modifier le Planning ({activeTab})</h3>
                <div className="space-y-8">
                  <input autoFocus type="text" value={newHorse.nom} onChange={e => setNewHorse({...newHorse, nom: e.target.value})} placeholder="NOM DU CHEVAL" className="w-full bg-white/5 p-5 rounded-3xl font-black text-xl border-2 border-transparent focus:border-[#8DC63F] outline-none uppercase transition-all" />
                    ...
                    <div className="bg-white p-5 rounded-[30px] flex flex-wrap gap-2 shadow-sm border border-gray-100">
                       {(activeTab === 'proprios' ? chevauxProprios.filter(s => (s.jour || "").toLowerCase() === j) : chevauxClub.filter(c => c.planning?.[j])).map(s => (
                         <div key={s.id} className="bg-gray-50 pl-4 pr-2 py-2 rounded-full flex items-center gap-2 border border-gray-100 animate-in fade-in">
                           ...
                           {s.lieu && s.lieu !== 'Sortie' && <span className="text-[8px] opacity-40 font-bold uppercase italic">{s.lieu}</span>}
