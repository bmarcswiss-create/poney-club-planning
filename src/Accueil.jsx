...
const Accueil = ({ onNavigate }) => {
  ...
  const TaskCard = ({ t, isActive }) => (
    <div className={`flex items-center justify-between p-4 rounded-3xl border-2 transition-all shadow-sm ${isActive ? 'bg-white border-white' : 'bg-white/40 border-transparent opacity-60'}`}>
      <div className="flex items-center gap-4 flex-1" onClick={() => toggleTache(t.id, t.est_fait, isActive)}>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[9px] font-black uppercase text-[#8DC63F] mt-0.5">
            ...
            {t.section && !isActive && <span className="text-gray-400 ml-2">• Section {t.section}</span>}
          </span>
        </div>
      </div>
      <button onClick={(e) => { e.stopPropagation(); supprimerTache(t.id); }} className="text-gray-200 hover:text-red-400 p-2"><Trash2 size={16} /></button>
    </div>
  );
