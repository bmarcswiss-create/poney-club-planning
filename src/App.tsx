...
export default function App() {
  ...
  const [isDirectionAuthenticated, setIsDirectionAuthenticated] = useState(false);
            ...
            {jours.map((dateObj, idx) => {
              ...
              return (
                <div key={dStr} onClick={() => { setSelectedDate(dStr); setModalChoiceOpen(dStr); }}
                  ...
                  <span className={`z-10 font-bold text-[11px] md:text-[13px] ${(pres.total < 4 || (absVis.length > 0 && pres.total >= 4)) ? 'text-white' : isPast ? 'text-gray-500' : 'text-[#1B2A49]'}`}>{dateObj.getDate()}</span>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#F0F4F8] text-[#1B2A49] overflow-hidden font-sans">
      ...
      {/* SIDEBAR */}
      <aside className="w-full md:w-80 bg-[#1B2A49] flex flex-col shrink-0 shadow-xl text-white text-center z-30">
        <div className="p-4 md:p-6 bg-[#141D36] flex flex-row md:flex-col items-center justify-between md:justify-center border-b-4 border-[#8DC63F]">
          <img src={LOGO_URL} alt="Logo" className="w-12 h-12 md:w-24 md:h-24 bg-white rounded-full p-1" />
          <div className="flex flex-col items-center flex-1 md:mt-2">
             <div className="flex items-center gap-2 mb-1">
                <h2 className="text-[9px] font-extrabold uppercase bg-[#8DC63F] text-[#1B2A49] px-2 py-0.5 rounded">Équipes PCP</h2>
             ...
             <h1 className="text-sm md:text-xl font-bold uppercase leading-none md:mt-1 font-black tracking-tight">Poney Club Presinge</h1>
          </div>
        </div>
        ...
        <div className="p-3 md:p-4 space-y-3">
           {isAdmin && (
             <>
                <button onClick={() => { setFormData({ ...formData, dateDebut: selectedDate, dateFin: selectedDate, category: 'conge' }); setModalCongeOpen(true); }} className="w-full bg-[#8DC63F] text-[#1B2A49] font-black py-3 rounded-xl flex items-center justify-center gap-2 uppercase text-[10px] md:text-xs shadow-lg active:scale-95"><Plus size={16}/> Absence</button>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => { setEvtForm({ ...evtForm, dateDebut: selectedDate }); setModalEvtOpen(true); }} className="bg-blue-600 text-white py-2 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-1"><Calendar size={12}/> Event</button>
           ...
           <button onClick={() => setFiltreEmploye(null)} className={`w-full bg-white/10 text-white font-bold py-2.5 rounded-xl text-[10px] md:text-xs ${!filtreEmploye ? 'ring-2 ring-[#8DC63F]' : ''}`}><FilterX size={14} className="inline mr-1"/> TOUTE L'ÉQUIPE</button>
        </div>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          {isAdmin && <button onClick={() => setModalStaffOpen(true)} className="w-full p-2 bg-white/5 hover:bg-white/10 rounded-lg text-[9px] uppercase font-bold flex items-center justify-center gap-2"><UserCog size={12}/> Paramètres Staff</button>}
          <button onClick={() => { setDirPinInput(""); setModalDirOpen(true); }} className="w-full p-2 bg-purple-600/30 hover:bg-purple-600/50 rounded-lg text-[9px] uppercase font-bold flex items-center justify-center gap-2 transition-all"><ShieldCheck size={12}/> Accès Direction</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50">
        ...
        <header className="bg-white border-b flex flex-col items-center justify-center px-4 py-4 md:h-44 shrink-0 gap-2">
          <div className="flex items-center gap-4">
            <button onClick={() => setAnneeActuelle(a => a-1)} className="p-2 border rounded-lg bg-gray-50 hover:bg-gray-100"><ChevronLeft size={18}/></button>
            <h2 className="text-xl md:text-2xl font-black tracking-tighter text-[#1B2A49]">{anneeActuelle}</h2>
            <button onClick={() => setAnneeActuelle(a => a+1)} className="p-2 border rounded-lg bg-gray-50 hover:bg-gray-100"><ChevronRight size={18}/></button>
          </div>
          
          <div className={`w-full max-w-xl rounded-2xl border-2 flex flex-col justify-center items-center p-3 transition-all shadow-lg ${selectedDate === todayStr ? 'border-orange-500' : 'border-[#D0D7E1]'} ${getDayPresence(selectedDate).total < 4 ? 'bg-red-600 text-white animate-pulse' : 'bg-white'}`}>
            <h3 className="text-xs md:text-base font-black uppercase text-center mb-1">{new Date(selectedDate).toLocaleDateString('fr-CH', {weekday:'long', day:'numeric', month:'long'})}</h3>
            ...
            <div className="flex flex-col items-center w-full">
                <div className="flex items-center gap-4 bg-[#1B2A49] px-6 py-1.5 md:px-8 rounded-full text-white shadow-md mb-2">
                ...
                {getDayPresence(selectedDate).absentsPlanifies.length > 0 && (
                   <div className="flex items-center gap-2 text-[9px] font-bold text-gray-400 italic">
                      <Info size={12}/> {getDayPresence(selectedDate).absentsPlanifies.map((a, i) => <span key={i}>{a.nom} ({a.type === 'conge' ? '🏝️' : '✈️'}){i < getDayPresence(selectedDate).absentsPlanifies.length - 1 ? ', ' : ''}</span>)}
                   </div>
                )}
            </div>
          </div>
        ...
        <div className="bg-white px-4 py-2 flex justify-center gap-3 md:gap-8 border-b text-[8px] md:text-[9px] font-black uppercase tracking-wider shrink-0 overflow-x-auto whitespace-nowrap">
          ...
          <div className="flex items-center gap-1"><div className="w-4 h-[3px] bg-purple-600"></div> Fériés / GE</div>
          <div className="flex items-center gap-1"><div className="w-4 h-[3px] bg-blue-500"></div> Club</div>
        </div>
      ...
      {modalChoiceOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[500] p-4 text-[#1B2A49]">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-5 space-y-4 border-t-8 border-[#8DC63F] shadow-2xl relative text-center max-h-[90vh] overflow-y-auto">
            ...
            {isAdmin && (
              <div className="space-y-2 pt-2">
                  <button onClick={() => { setFormData({ ...formData, dateDebut: modalChoiceOpen, dateFin: modalChoiceOpen, category: 'conge' }); setModalCongeOpen(true); setModalChoiceOpen(null); }} className="w-full py-4 bg-[#8DC63F] text-[#1B2A49] font-black rounded-2xl uppercase text-xs shadow-md active:scale-95">Absence / Maladie</button>
                  <div className="grid grid-cols-2 gap-2">
      ...
      {/* MODALE DIRECTION - ACCÈS SÉCURISÉ */}
      {modalDirOpen && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[800] p-4 text-[#1B2A49]">
          <div className="bg-white rounded-[2rem] w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl border-t-8 border-purple-600">
            <div className="p-6 bg-purple-600 text-white flex justify-between items-center shrink-0">
              <h2 className="text-xl font-black uppercase flex items-center gap-2"><ShieldCheck/> Direction - Analyses PCP 2026</h2>
              <X onClick={() => { setIsDirectionAuthenticated(false); setModalDirOpen(false); }} className="cursor-pointer"/>
            </div>
            
            {!isDirectionAuthenticated ? (
              <div className="p-20 text-center space-y-6">
                <input autoFocus type="password" placeholder="Mot de passe direction" className="w-full max-w-xs text-center text-2xl border-b-4 border-purple-600 p-3 outline-none" value={dirPinInput} onChange={e => setDirPinInput(e.target.value)} onKeyDown={e => { if(e.key==='Enter' && dirPinInput === PIN_DIRECTION) setIsDirectionAuthenticated(true); }}/>
                <button onClick={() => { if(dirPinInput === PIN_DIRECTION) setIsDirectionAuthenticated(true); else alert("Accès refusé"); }} className="block mx-auto bg-purple-600 text-white px-8 py-3 rounded-xl font-bold uppercase active:scale-95 transition-all">Accéder</button>
              </div>
            ) : (
              <div className="flex-1 overflow-y-auto p-6 bg-gray-50 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="bg-white p-6 rounded-3xl border shadow-sm">
                      <div className="text-xs font-black uppercase mb-4 flex items-center gap-2 text-red-600"><AlertTriangle size={16}/> Criticité par Jour de Semaine</div>
                      <div className="space-y-3">
                         {JOURS_SEMAINE.map((jour, idx) => {
                            const manques = [12, 5, 8, 18, 4, 25, 30][idx];
                            return (
                               <div key={jour} className="flex items-center gap-4">
                                  <span className="w-20 text-[10px] font-black uppercase opacity-60">{jour}</span>
                                  <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                                     <div className={`h-full ${manques > 20 ? 'bg-red-500' : 'bg-green-500'}`} style={{width: `${manques}%`}}></div>
                                  </div>
                                  <span className="text-[10px] font-black">{manques}% risque</span>
                               </div>
                            )
                         })}
                      </div>
                   ...
                   <div className="bg-white p-6 rounded-3xl border shadow-sm">
                      <div className="text-xs font-black uppercase mb-6 flex justify-between items-center">Taux de Présence Mensuelle (%) <TrendingUp size={14}/></div>
                      <div className="flex items-end gap-3 h-32">
                         {[85, 88, 92, 70, 95, 85, 90, 80, 75, 88, 85, 90].map((v, i) => (
                           <div key={i} className="flex-1 bg-[#8DC63F] rounded-t-lg relative group transition-all" style={{height: `${v}%`}}>
                             <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[9px] font-black bg-black text-white px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">{v}%</div>
                           </div>
                         ))}
                      </div>
                      <div className="flex justify-between mt-3 text-[8px] font-black opacity-30 uppercase">
                        ...
                        return (
                          <tr key={m.id} className="hover:bg-purple-50/30 transition-colors">
                            ...
                            <td className="p-4 text-center border-r">
                              <input type="number" className="w-16 border rounded p-1 text-center bg-gray-50" value={m.quotaVacances || 25} onChange={async (e) => { 
                            ...
                            <td className={`p-4 text-center border-r text-lg ${solde < 5 ? 'text-red-600 font-black' : 'text-green-600'}`}>{solde} j</td>
      ...
      {/* AUTRES MODALES */}
      {modalPinOpen && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[700] p-4 text-[#1B2A49]"><div className="bg-white rounded-3xl p-8 w-full max-w-sm text-center shadow-2xl"><Lock className="mx-auto mb-4" size={48} /><form onSubmit={(e) => { e.preventDefault(); if (pinInput.toLowerCase() === PIN_ADMIN) { setIsAdmin(true); setModalPinOpen(false); setPinInput(""); } else { alert("Code incorrect"); } }} className="space-y-4"><input autoFocus type="password" placeholder="PIN" className="w-full text-center text-3xl border-b-4 border-[#1B2A49] p-3 outline-none" value={pinInput} onChange={e => setPinInput(e.target.value)} /><button type="submit" className="w-full py-4 bg-[#1B2A49] text-white rounded-xl font-bold uppercase active:scale-95 transition-all">Déverrouiller</button></form></div></div>
      )}
      ...
      {modalCongeOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[600] p-4 text-[#1B2A49]">
          <div className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl border-2 border-[#1B2A49]">
            <div className="p-5 bg-[#1B2A49] text-white flex justify-between items-center font-black uppercase text-sm">Nouvelle Absence<X onClick={() => setModalCongeOpen(false)} className="cursor-pointer"/></div>
            <form onSubmit={async (e) => { e.preventDefault(); const nC = [...conges]; const s = new Date(formData.dateDebut); const en = new Date(formData.dateFin || formData.dateDebut); const g = Date.now(); for (let d = new Date(s); d <= en; d.setDate(d.getDate() + 1)) { nC.push({ id: Math.random(), groupId: g, userId: formData.userId, date: d.toLocaleDateString('en-CA'), statut: formData.category === 'maladie' ? 'valide' : formData.statut, category: formData.category, periode: formData.periode }); } setConges(nC); setModalCongeOpen(false); await syncAll(membresBase, nC, notes, evenementsPerso, manualPresence); }} className="p-6 space-y-4">
              <select className="w-full border-2 p-4 rounded-xl font-black text-sm" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3"><input type="date" className="border-2 p-4 rounded-xl font-bold text-sm" value={formData.dateDebut} onChange={e => setFormData({...formData, dateDebut: e.target.value})} required/><input type="date" className="border-2 p-4 rounded-xl font-bold text-sm" value={formData.dateFin} onChange={e => setFormData({...formData, dateFin: e.target.value})}/></div>
              <select className="w-full border-2 p-4 rounded-xl font-black text-sm" value={formData.userId} onChange={e => setFormData({...formData, userId: e.target.value})} required><option value="">-- Choisir employé --</option>{membresBase.map(m => <option key={m.id} value={m.id}>{m.nom}</option>)}</select>
              <button type="submit" className="w-full bg-[#1B2A49] text-white py-5 rounded-2xl font-black uppercase text-sm active:scale-95 transition-all">Confirmer</button>
            </form>
          </div>
        </div>
      )}
      ...
      {modalStaffOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[600] p-2 text-[#1B2A49]">
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl">
            <div className="bg-[#1B2A49] p-5 text-white flex justify-between items-center uppercase font-bold text-sm">Gestion Staff PCP<X onClick={() => setModalStaffOpen(false)} className="cursor-pointer"/></div>
            <div className="flex flex-col md:flex-row p-4 gap-6 max-h-[80vh] overflow-y-auto">
               <form onSubmit={async (e) => { e.preventDefault(); const nI = staffForm.id || Date.now().toString(); const up = staffForm.id ? membresBase.map(m => m.id === staffForm.id ? staffForm : m) : [...membresBase, { ...staffForm, id: nI }]; setMembresBase(up); setModalStaffOpen(false); await syncAll(up, conges, notes, evenementsPerso, manualPresence); }} className="flex-1 space-y-4 text-left"><label className="block text-[10px] font-black uppercase opacity-50 ml-1">Nom complet</label><input className="w-full border-2 p-4 rounded-xl font-bold text-[#1B2A49]" value={staffForm.nom} onChange={e => setStaffForm({...staffForm, nom: e.target.value})} required placeholder="Nom"/><select className="w-full border-2 p-4 rounded-xl font-black text-[#1B2A49]" value={staffForm.role} onChange={e => setStaffForm({...staffForm, role: e.target.value})}>{["Palefrenier", "Apprentie", "Monitrice", "Aide WE", "Aide ponctuel"].map(r => <option key={r} value={r}>{r}</option>)}</select><button type="submit" className="w-full bg-[#1B2A49] text-white py-4 rounded-xl font-black uppercase active:scale-95 transition-all">Sauvegarder</button></form>
      ...
      {modalNoteOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[600] p-4 text-[#1B2A49]">
          <div className="bg-white rounded-[2rem] w-full max-w-sm overflow-hidden border-4 border-yellow-400 shadow-2xl text-center">
            <div className="p-4 bg-yellow-400 font-black flex justify-between uppercase italic text-xs">Post-it PCP<X onClick={() => setModalNoteOpen(false)} className="cursor-pointer"/></div>
            <form onSubmit={async (e) => { e.preventDefault(); const nx = { ...notes }; if (!noteText.trim()) delete nx[selectedDate]; else nx[selectedDate] = noteText; setNotes(nx); setModalNoteOpen(false); await syncAll(membresBase, conges, nx, evenementsPerso, manualPresence); }} className="p-6 space-y-4"><textarea autoFocus className="w-full border-2 border-yellow-100 p-5 rounded-2xl bg-yellow-50 outline-none h-40 font-bold text-[#1B2A49]" value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Message..."/><button type="submit" className="w-full bg-yellow-400 text-[#1B2A49] font-black py-4 rounded-xl uppercase active:scale-95 transition-all">Enregistrer</button></form>
          </div>
        </div>
      )}
      ...
      {modalEvtOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[600] p-4 text-[#1B2A49]">
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl relative text-center"><X className="absolute top-4 right-4 cursor-pointer text-gray-400" onClick={() => setModalEvtOpen(false)}/><h3 className="font-black uppercase mb-6 text-[#1B2A49]">Event PCP</h3><form onSubmit={async (e) => { e.preventDefault(); const nx = { ...evenementsPerso }; const date = evtForm.dateDebut; nx[date] = [...(nx[date] || []), { id: Date.now(), titre: evtForm.titre, type: evtForm.type }]; setEvenementsPerso(nx); setModalEvtOpen(false); await syncAll(membresBase, conges, notes, nx, manualPresence); }} className="space-y-4"><input type="date" className="w-full border-2 p-4 rounded-xl font-bold text-[#1B2A49]" value={evtForm.dateDebut} onChange={e => setEvtForm({...evtForm, dateDebut: e.target.value})}/><input type="text" className="w-full border-2 p-4 rounded-xl font-black text-[#1B2A49]" placeholder="Nom event" value={evtForm.titre} onChange={e => setEvtForm({...evtForm, titre: e.target.value})} required/><button type="submit" className="w-full bg-[#1B2A49] text-[#8DC63F] py-4 rounded-xl font-black uppercase active:scale-95 transition-all">Ajouter</button></form></div>
        </div>
      )}
