...
import { Calendar as CalendarIcon, MapPin, Award, Trash2, X, Plus, AlertCircle, Users, Tent, ChevronLeft, ChevronRight, Settings, Edit, Flag, StickyNote, Printer, FilterX, CheckCircle2 } from 'lucide-react';
...
export default function App() {
  ...
  // --- SAUVEGARDE (VERROU ANTI-ECRASEMENT) ---
  const sync = (key, val) => {
    if (!isLoaded) return;
  ...
  useEffect(() => { sync('poney_equipe', membresBase); }, [membresBase]);
  useEffect(() => { sync('poney_conges', conges); }, [conges]);
  useEffect(() => { sync('poney_evenements', evenements); }, [evenements]);
  useEffect(() => { sync('poney_notes', notes); }, [notes]);
  useEffect(() => { sync('poney_annee', anneeActuelle.toString()); }, [anneeActuelle]);
  ...
  const absentsSel = congesParDate[selectedDate] || [];
  const evtsSel = evenements[selectedDate] || [];
  const isAlerteHeader = !filtreEmploye && absentsSel.filter(c => palefrenierIds.has(c.userId)).length > 2;
  ...
  const validateConge = (id) => {
    setConges(conges.map(c => c.id === id ? { ...c, statut: 'valide' } : c));
  };

  const validatePeriod = (groupId) => {
    setConges(conges.map(c => c.groupId === groupId ? { ...c, statut: 'valide' } : c));
    setModalChoiceOpen(null);
  };
  ...
  const deletePeriod = (groupId) => {
    if (window.confirm("Supprimer toute la période ?")) {
      setConges(conges.filter(c => c.groupId !== groupId));
      setModalChoiceOpen(null);
    }
  };
            ...
            {jours.map((j, idx) => {
              ...
              return (
                <div key={dStr} onClick={() => { setSelectedDate(dStr); setModalChoiceOpen(dStr); }}
                  ...
                  ${isAlerte ? 'bg-red-500 text-white font-bold shadow-inner' : isToday ? 'bg-amber-100 ring-2 ring-amber-500 z-10' : (isPast && !hasConge && evts.length === 0 ? 'bg-[#E2E8F0] opacity-50' : 'bg-[#F4F6F9]')} print:border print:border-gray-100`}>
                    ...
                    <div className="absolute inset-0 flex flex-col opacity-100">
                      {[0, 1].map(h => {
                         ...
                         return <div key={h} className={`h-1/2 w-full relative ${bg}`}>
                           {a.statut === 'provisoire' && a.category !== 'deplacement' && <div className="absolute inset-0 striped-provisoire" style={{background: 'repeating-linear-gradient(45deg, #1B2A49, #1B2A49 2px, #22c55e 2px, #22c55e 4px)', opacity: 0.9}}></div>}
                         </div>;
                      })}
                    </div>
                  )}
      ...
      <div className="flex h-screen flex-mobile overflow-hidden print:block print:h-auto">
        ...
        <aside className="w-80 side-mobile bg-[#1B2A49] flex flex-col print:hidden shrink-0 shadow-xl relative z-50 text-white font-sans">
          <div className="p-6 bg-[#141D36] flex flex-col items-center border-b-4 border-[#8DC63F]">
            ...
            <h2 className="text-[10px] font-extrabold uppercase bg-[#8DC63F] text-[#1B2A49] px-3 py-1 rounded shadow-sm">Organisation des équipes écuries</h2>
          </div>

          <div className="p-4 bg-[#141D36]/50 space-y-3 shrink-0">
             <button onClick={() => setModalStaffOpen(true)} className="w-full bg-[#1B2A49] text-[#8DC63F] border border-[#8DC63F]/30 font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-2 transition-all"><Settings size={14}/> Configuration Équipe</button>
        ...
        {/* MODAL CHOICE AVEC VALIDATION RAPIDE */}
        {modalChoiceOpen && (
          <div className="fixed inset-0 bg-black/70 flex items-end md:items-center justify-center z-[110] p-4 print:hidden text-[#1B2A49]">
            <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-sm p-6 space-y-4 border-t-8 border-[#8DC63F] shadow-2xl relative">
              ...
              <button onClick={() => { setFormData({ ...formData, dateDebut: modalChoiceOpen, dateFin: modalChoiceOpen }); setModalCongeOpen(true); setModalChoiceOpen(null); }} className="w-full py-4 bg-[#8DC63F] text-[#1B2A49] font-black rounded-2xl flex items-center justify-center gap-2 shadow-md hover:scale-105 transition-transform uppercase">Absence / Déplacement</button>
              ...
              <div className="pt-2 max-h-40 overflow-y-auto space-y-1 text-center">
                 ...
                 {(congesParDate[modalChoiceOpen] || []).map(a => (
                   <div key={a.id} className="flex gap-1 text-[#1B2A49]">
                      <div className="flex-1 flex flex-col bg-gray-50 border rounded-lg p-2 text-left">
                        <span className="text-[11px] font-black">{membresBase?.find(u=>u.id===a.userId)?.nom}</span>
                        <span className="text-[9px] uppercase opacity-50">{a.category} - {a.statut}</span>
                        <div className="flex gap-2 mt-1">
                          <button onClick={() => deleteConge(a.id)} className="text-red-600 text-[9px] font-black uppercase flex items-center gap-1"><Trash2 size={10}/> Jour</button>
                          {a.groupId && <button onClick={() => deletePeriod(a.groupId)} className="text-red-600 text-[9px] font-black uppercase border-l pl-2">Période</button>}
                          {a.statut === 'provisoire' && (
                            <button onClick={() => a.groupId ? validatePeriod(a.groupId) : validateConge(a.id)} className="text-green-600 text-[9px] font-black uppercase border-l pl-2 flex items-center gap-1"><CheckCircle2 size={10}/> Valider</button>
                          )}
                        </div>
                      </div>
                   </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[110] p-4 text-[#1B2A49]">
             <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden border-4 border-yellow-400 shadow-2xl">
                ...
                <form onSubmit={handleSaveNote} className="p-6 space-y-4">
                   ...
                   <div className="flex gap-2 text-[#1B2A49]">
